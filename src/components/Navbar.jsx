...
export default function Navbar() {
  ...
  return (
    <header className="bg-black sticky top-0 z-50 w-full">
      ...
      {/* Mobile Menu */}
      {mobileMenuOpen && (
        <div className="md:hidden fixed top-0 right-0 h-1/3 w-1/4 bg-mpf-black backdrop-blur-sm p-6 flex flex-col gap-6 text-center font-arial text-2xl uppercase text-mpf-yellow rounded-bl-lg items-end z-50 shadow-xl not-italic">
          {navLinks.map((link) => (
            <a
              key={link.label}
              href={link.href}
